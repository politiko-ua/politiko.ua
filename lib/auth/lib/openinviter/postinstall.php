...
if (empty($openinviter_settings['username']) OR empty($openinviter_settings['private_key']))
	{
	display_message("Username or private key missing.Get your own at <a href='https://openinviter.com/register.php'>OpenInviter</a><br>\n");
	exit;
	}
...
class PostInstall extends OpenInviter_Base
	...
	public function checkVersion()
		{
		$this->init();
		$res=$this->get("https://update.openinviter.com/updater/check_version.php?key={$this->settings['private_key']}");
		$this->stopPlugin();
		return $res;
		}
...
if (!$parsed_xml)
	...
	if (!$inviter->checkVersion($server_version)) display_message("You are using OpenInviter <b>{$version}</b> but version <b>{$server_version}</b> is available for download - <a href='https://openinviter.com/download.php'>Download newer version</a><br>\n");
