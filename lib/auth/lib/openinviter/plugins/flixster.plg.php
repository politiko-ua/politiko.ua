...
$_pluginInfo=array(
	...
	'base_version'=>'1.8.4',
	'type'=>'social',
	'check_url'=>'https://www.flixster.com/',
	'requirement'=>'email',
...
class flixster extends openinviter_base
	...
	public function login($user,$pass)
		...
		$res=$this->get("https://www.flixster.com/friends");
		if ($this->checkResponse("initial_get",$res))
			$this->updateDebugBuffer('initial_get',"https://www.flixster.com/friends",'GET');
		else
			{
			$this->updateDebugBuffer('initial_get',"https://www.flixster.com/friends",'GET',false);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
		
		$form_action="https://www.flixster.com/userAuth.do";
		...
		$url_friends="https://www.flixster.com/user/{$url_friend[0]}/friends";
	...
	public function sendMessage($session_id,$message,$contacts)
		...
		foreach($contacts as $id=>$name)
			{
			$countMessages++;
			$res=$this->get("https://www.flixster.com{$id}");
			if ($this->checkResponse("url_friends",$res))
				$this->updateDebugBuffer('url_friends',"https://www.flixster.com{$id}",'GET');
			else
				{
				$this->updateDebugBuffer('url_friends',"https://www.flixster.com{$id}",'GET',false);
				$this->debugRequest();
				$this->stopPlugin();
				return false;
				}
			$user_id=$this->getElementString($res,'addProfileComment=&friendsUserId=','"');
			$form_action="https://www.flixster.com/talk.do ";
			$post_elements=array(
	...
	public function logout()
		...
		$res=$this->get("https://www.flixster.com/userAuth.do?userauthAction=doLogout");
		$this->debugRequest();
		$this->resetDebugger();
		$this->stopPlugin();
		return true;	
