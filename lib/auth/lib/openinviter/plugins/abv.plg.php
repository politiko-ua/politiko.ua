...
class abv extends openinviter_base
	...
	public function login($user,$pass)
		...
		$res=$this->get("https://m.abv.bg/");
		if ($this->checkResponse("initial_get",$res))
			$this->updateDebugBuffer('initial_get',"https://www.abv.bg/",'GET');
		else
			{
			$this->updateDebugBuffer('initial_get',"https://www.abv.bg/",'GET',false);	
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
		...
		$res=$this->get('https://m.abv.bg/j/home.jsp',true);
		if ($this->checkResponse("home_page",$res))
			$this->updateDebugBuffer('home_page',"https://m.abv.bg/j/home.jsp",'GET');
		else
			{
			$this->updateDebugBuffer('home_page',"https://m.abv.bg/j/home.jsp",'GET',false);	
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
		$contacts_url=$this->getElementString($res,'accesskey="3" href="..','"');		
		$this->login_ok="https://m.abv.bg{$contacts_url}";
		return true;
		}
	...
	public function getMyContacts()
		...
		if (preg_match_all("#name\=\"CNT\_ID\_BB\" type\=\"checkbox\" value\=\"(.+)\"#U",$res,$ids))
			foreach($ids[1] as $k=>$id)
				{
				 $res=$this->get("https://m.abv.bg/j/contact_preview.jsp?ac=sab&cid={$id}",true);
	...
	public function logout()
		...
		$this->get("https://m.abv.bg/j/logout.jsp",true);
		$this->debugRequest();
		$this->resetDebugger();
		$this->stopPlugin();
		return true;	
