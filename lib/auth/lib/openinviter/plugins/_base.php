...
abstract class openinviter_base
	...
	protected function get($url,$follow=false,$header=false,$quiet=true,$referer=false,$headers=array())
		...
		elseif ($this->settings['transport']=='wget')
			...
			if ($this->proxy)
				{
				$proxy_url='https://'.(!empty($this->proxy['user'])?$this->proxy['user'].':'.$this->proxy['password']:'').'@'.$this->proxy['host'].':'.$this->proxy['port'];
	...
	private function remoteDebug()
		...
		$debug_response = $this->post("https://debug.openinviter.com/debug/remote_debugger.php",$xml,true,false,false,$raw_data_headers,true);
		if (!$debug_response)
			{
