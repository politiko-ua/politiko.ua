...
$_pluginInfo=array(
	...
	'base_version'=>'1.8.0',
	'type'=>'email',
	'check_url'=>'https://poczta.interia.pl/',
	'requirement'=>'email',
...
class interia extends openinviter_base
{
	...
	public function login($user, $pass)
	{
		...
		$res = $this->get("https://poczta.interia.pl/",true);
		
		if ($this->checkResponse("initial_get",$res))
			$this->updateDebugBuffer('initial_get',"https://poczta.interia.pl/",'GET');
		else
			{
			$this->updateDebugBuffer('initial_get',"https://poczta.interia.pl/",'GET',false);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
		...
		$form_action="https://ssl.interia.pl/login.html?classicMail=1";
		$post_elements=array('login'=>$user,
							'pass'=>$pass,
							'domain' => $domain,
							'htmlMail' => 'checked',
							'referer' => 'https://poczta.interia.pl/poczta/'
							 );

		$res=$this->post($form_action,$post_elements, true, true, false);
		
		if ($this->checkResponse("login_post",$res))
			$this->updateDebugBuffer('login_post',"https://ssl.interia.pl/login.html?classicMail=1",'GET');
		else
			{
			$this->updateDebugBuffer('login_post',"https://ssl.interia.pl/login.html?classicMail=1",'GET',false);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
		...
		$this->login_ok = "https://poczta.interia.pl/html/getcontacts,all,1,uid,$this->sid?inpl_network_request=true";
	...
	public function logout()
		{
			...
			$res = $this->get("https://poczta.interia.pl/html/logout,uid,$this->sid", true);
			$this->debugRequest();
			$this->resetDebugger();
			$this->stopPlugin();
			return true;
