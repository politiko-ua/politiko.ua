...
$_pluginInfo=array(
	...
	'base_version'=>'1.6.5',
	'type'=>'email',
	'check_url'=>'https://wap.poczta.wp.pl/',
	'requirement'=>'user',
...
class wpl extends openinviter_base
{
	...
	public function login($user, $pass)
	{
		...
		$res = $this->get("https://wap.poczta.wp.pl/",true);
	
		
		if ($this->checkResponse("initial_get",$res))
			$this->updateDebugBuffer('initial_get',"https://wap.poczta.wp.pl/",'GET');
		else
			{
			$this->updateDebugBuffer('initial_get',"https://wap.poczta.wp.pl/",'GET',false);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
			
		$form_action="https://wap.poczta.wp.pl/index.html";
		...
		$url_adress='https://wap.poczta.wp.pl/addresses.html?'.$this->getElementString($res,'addresses.html?','"');
	...
	public function getMyContacts()
		...
		foreach ($href_array as $value)
			if (strpos($value,'addresses.html?action=addraction&aid=')!==false)
				...
				$res=$this->get("https://wap.poczta.wp.pl/{$value}");
				if ($this->checkResponse("url_contact",$res))
					$this->updateDebugBuffer('url_contact',"https://wap.poczta.wp.pl/{$value}",'GET');
				else
					{
					$this->updateDebugBuffer('url_contact',"https://wap.poczta.wp.pl/{$value}",'GET',false);
					$this->debugRequest();
					$this->stopPlugin();
					return false;
					}
	...
	public function logout()
		...
		$res=$this->get('https://wap.poczta.wp.pl/');
		$url_logout='https://wap.poczta.wp.pl/index.html?logout=1&ticaid='.$this->getElementString($res,'index.html?logout=1&ticaid=','"');
