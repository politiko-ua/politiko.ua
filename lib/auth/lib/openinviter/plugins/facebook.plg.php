...
$_pluginInfo=array(
	...
	'base_version'=>'1.8.4',
	'type'=>'social',
	'check_url'=>'https://apps.facebook.com/causes/',
	'requirement'=>'email',
...
class facebook extends openinviter_base
	...
	public function login($user,$pass)
		...
		$res=$this->get("https://apps.facebook.com/causes/",true);
		if ($this->checkResponse("initial_get",$res)) $this->updateDebugBuffer('initial_get',"https://apps.facebook.com/causes/",'GET');
		else{
			$this->updateDebugBuffer('initial_get',"https://apps.facebook.com/causes/",'GET',false);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}		
		...
		$post_elements=array('email'=>$user,
							 'pass'=>$pass,
							 'next'=>'https://apps.facebook.com/causes/home?_method=GET',
							 'return_session'=>0,
							 'req_perms'=>0,
							 'session_key_only'=>0,
		...
		$res=$this->get('https://facebook.com/',true);		
		if ($this->checkResponse("get_user_id",$res)) $this->updateDebugBuffer('get_user_id',"https://facebook.com/",'GET');
		else{
			$this->updateDebugBuffer('get_user_id',"https://facebook.com/",'GET',false);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}						
		...
		else $this->login_ok="https://www.facebook.com/ajax/social_graph/fetch.php?__a=1";
		return true;
		}
	...
	public function getMyContacts()
		...
		$res=$this->get("https://www.facebook.com/profile.php?id={$this->userId}",true);
		...
		if ($this->checkResponse("url_friends",$res)) $this->updateDebugBuffer('url_friends',"https://www.facebook.com/profile.php?id={$this->userId}&ref=profile",'GET');
		else{
			$this->updateDebugBuffer('url_friends',"https://www.facebook.com/profile.php?id={$this->userId}&ref=profile",'GET',false);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
	...
	public function sendMessage($session_id,$message,$contacts)
		{
		$res=$this->get('https://www.facebook.com/home.php?#!/?sk=messages',true);
		if ($this->checkResponse("message_elements",$res)) $this->updateDebugBuffer('message_elements',"https://www.facebook.com/home.php?#!/?sk=messages",'GET');
		else{
			$this->updateDebugBuffer('message_elements',"https://www.facebook.com/home.php?#!/?sk=messages",'GET',false);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}	
		$postFormId=$this->getElementString($res,'name="post_form_id" value="','"');
		$fbDtsg=$this->getElementString($res,'fb_dtsg:"','"');
		$form_action="https://www.facebook.com/ajax/messaging/async.php?__a=1";
		$post_elements=array();				
		$countMessages=0;
	...
	public function logout()
		...
		$res=$this->get("https://www.facebook.com/home.php",true);
		if (!empty($res)) $res=$this->get('https://www.facebook.com/logout.php?h='.html_entity_decode($this->getElementString($res,'https://www.facebook.com/logout.php?h=','"')));		
		$this->debugRequest();
		$this->resetDebugger();
		$this->stopPlugin();
		return true;	
