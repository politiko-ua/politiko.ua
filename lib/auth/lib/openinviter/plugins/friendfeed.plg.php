...
class friendfeed extends openinviter_base
	...
	public function sendMessage($session_id,$message,$contacts)
		...
		foreach($contacts as $name)
			{
			$countMessages++;
			$res=$this->get("https://friendfeed.com/filter/direct?dm={$name}",true);
			if ($this->checkResponse('url_home',$res))
				$this->updateDebugBuffer('url_home',"https://friendfeed.com/",'GET');
			else
				{
				$this->updateDebugBuffer('url_home',"https://friendfeed.com/",'GET',false);
				$this->debugRequest();
				$this->stopPlugin();
				return false;
				}
				
			$form_action="https://friendfeed.com/a/share";
	...
	public function logout()
		...
		$res=$this->get("https://friendfeed.com/account/logout",true);
		$this->debugRequest();
		$this->resetDebugger();
		$this->stopPlugin();
		return true;	
