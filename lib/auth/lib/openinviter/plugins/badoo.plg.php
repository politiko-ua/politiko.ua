...
class badoo extends openinviter_base
	...
	public function login($user,$pass)
		...
		$res = $this->get('https://badoo.com/?lang_id=3',true);
		$url = $this->getElementString($res,'<a href="https://badoo.com/signin/','" class="sign_in">');
		$res = $this->get("https://badoo.com/signin/".$url,true);
		$post_elements=array();
		$post_elements['email']=$user;
		$post_elements['password']=$pass;
		$post_elements['post']='';
		$res = $this->post("https://badoo.com/signin/",$post_elements,true);
		if ($this->checkResponse("login_post",$res))
			$this->updateDebugBuffer('login_post',"https://badoo.com/signin/",'POST');		
		else
			{
			$this->updateDebugBuffer('login_post',"https://badoo.com/signin/",'POST',false);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
	...
	public function getMyContacts()
		...
		$res=$this->get("https://badoo.com/{$id}/contacts/subscriptions.phtml",true);
		if ($this->checkResponse("get_friends",$res))
			$this->updateDebugBuffer('get_friends',"https://badoo.com/{$id}/contacts/subscriptions.phtml",'GET');
		else
			{
			$this->updateDebugBuffer('get_friends',"https://badoo.com/{$id}/contacts/subscriptions.phtml",'GET',false);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
		...
		foreach($data as $node)
			...
			$href="https://badoo.com/{$id}/contacts/message/{$cId}";
	...
	public function sendMessage($session_id,$message,$contacts)
		...
		foreach($contacts as $url=>$name)
			...
			$master_id = $this->getElementString($url,'https://badoo.com/','/'); 
			$post_url = "https://badoo.com/{$master_id}/contacts/ws-post.phtml";
	...
	public function logout()
		...
		$res=$this->get("https://badoo.com/signout/");
		$this->debugRequest();
		$this->resetDebugger();
		$this->stopPlugin();
		return true;	
