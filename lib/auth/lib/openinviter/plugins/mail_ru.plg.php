...
class mail_ru extends openinviter_base
	...
	public function login($user,$pass)
		...
		$res=$this->get("https://www.mail.ru/",true);
		if ($this->checkResponse("initial_get",$res)) $this->updateDebugBuffer('initial_get',"https://www.mail.ru/",'GET');
		else{
			$this->updateDebugBuffer('initial_get',"https://www.mail.ru/",'GET',false);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}			
		...
		if ($this->checkResponse("login_post",$res)) $this->updateDebugBuffer('login_post',"https://win.mail.ru/cgi-bin/auth",'POST',true,$post_elements);
		else{
			$this->updateDebugBuffer('login_post',"https://win.mail.ru/cgi-bin/auth",'POST',false,$post_elements);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
		$url_export="https://e.mail.ru/cgi-bin/abexport/addressbook.csv";
	...
	public function logout()
		...
		$res=$this->get("https://e.mail.ru/cgi-bin/logout",true);
		$this->debugRequest();
		$this->resetDebugger();
		$this->stopPlugin();
		return true;	
