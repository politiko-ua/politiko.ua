...
class UploadForm {
	...
	private function curlCopy( $url, $dest ) {
		...
		$url =  trim( $url );
		if( stripos($url, 'https://') !== 0 && stripos($url, 'ftp://') !== 0 ) {
...
wgUploadAutoFill = {$autofill};
		...
		# See https://www.php.net/manual/en/ini.core.php#ini.upload-max-filesize for possible values of upload_max_filesize
