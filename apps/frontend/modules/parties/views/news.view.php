...
<div class="mr10">
	...
	<? foreach ( $list as $id ) { ?>
		<? $news_item = parties_news_peer::instance()->get_item($id); ?>
		<div id="news_head_<?=$id?>" class="mb5 quiet fs11 box_content p5">
			...
			<? if ( parties_peer::instance()->is_moderator($party['id'], session::get_user_id()) ) { ?>
				<a href="javascript:;" onclick="partiesController.editNews(<?=$id?>);" class="dotted ml10"><?=t('Редактировать')?></a>
				<a href="javascript:;" onclick="if ( confirm('<?=t('Удалить новость?')?>') ) partiesController.deleteNews(<?=$id?>);" class="dotted ml10"><?=t('Удалить')?></a>
			<? } ?>
		</div>
		<div class="m10 fs12" id="news_body_<?=$id?>">
			...
			<a href="/parties/newsread?id=<?=$news_item['id']?>"><?=t('Читать все &rarr;')?></a>
		</div>
	<? } ?>
...
<form id="edit_news_form" class="hidden" action="/parties/save_news">
