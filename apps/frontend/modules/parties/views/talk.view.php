<h1 class="mt10 mr10 column_head">
	<a href="/party<?=$party['id']?>"><?=htmlspecialchars($party['title'])?></a> &rarr <?=t('Обсуждения')?>
</h1>

<? if ( session::is_authenticated() ) { ?>
	<form class="form_bg mr10 fs12 p10 mb10" action="/parties/talk_create?id=<?=$party['id']?>" id="topic_form">
		<div class="left">
			<a class="filter <?=!$filter ? ' filter_selected' : ''?>" href="talk?id=<?=$party['id']?>"><?=t('Новые Темы')?></a>
			<a class="filter <?=$filter == 'hot' ? ' filter_selected' : ''?>" href="talk?id=<?=$party['id']?>&filter=hot"><?=t('Самые обсуждаемые')?></a>
		</div>
		<a href="javascript:;" onclick="$('#create_topic').show(50);" class="mt5 right dotted"><?=t('Создать тем')?>у</a>
		<div class="clear"></div>
		<div class="hidden" id="create_topic">
			...
			<table width="100%" class="mt10">
				...
				<tr>
					...
					<td><textarea style="width: 500px; height: 50px;" name="text" rel="<?=t('Введите текст темы')?>"></textarea></td>
				</tr>
				<tr>
					<td></td>
					<td>
...
<? } else { ?>
	<div id="no_questions" class="acenter fs12 p5 ml10"><?=t('Обсуждения еще не велись')?></div>
<? } ?>