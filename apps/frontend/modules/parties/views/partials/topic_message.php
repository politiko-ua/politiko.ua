<? $topic_message = parties_topics_messages_peer::instance()->get_item($id) ?>
<div class="mb10 comment_bg mr10" id="talk_message<?=$id?>">
	...
	<div class="left ml10" style="width: 525px;">
		...
		<? if ( ( $topic_message['user_id'] == session::get_user_id() ) || parties_peer::instance()->is_moderator($party['id'], session::get_user_id()) ) { ?>
			<div class="fs10 mt10">
				<a href="javascript:;" onclick="partiesController.deleteTalkMessage(<?=$id?>);" class="maroon dotted"><?=t('Удалить')?></a>
			</div>
		<? } ?>
	</div>
	<div class="clear"></div>
