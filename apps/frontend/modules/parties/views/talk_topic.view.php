<h1 class="mt10 mr10 column_head">
	<a href="/parties/talk?id=<?=$party['id']?>"><?=t('Обсуждения')?></a>
	&rarr;
	<?=t('Просмотр темы')?>
</h1>
...
<h3 class="mt5 mb10">
	...
	<? if ( ( $topic['user_id'] == session::get_user_id() ) || parties_peer::instance()->is_moderator($party['id'], session::get_user_id()) ) { ?>
		<a href="javascript:;" class="fs11 maroon" onclick="partiesController.deleteTalkTopic(<?=$topic['id']?>);"><?=t('Удалить тему')?></a>
	<? } ?>
</h3>
...
<? if ( session::is_authenticated() ) { ?>
	<form class="form_bg mr10 fs12 mb10" action="/parties/talk_message?id=<?=$topic['id']?>" id="message_form">
