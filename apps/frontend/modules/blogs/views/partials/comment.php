...
<div class="mb10 comment_bg" id="comment<?=$id?>" <?=db_key::i()->exists('negative_blog_comment:'.$id) ? 'style="background: #ffeeee; border: 1px solid #EEAAAA;"' : ''?>>
	...
	<div class="left ml10" style="width: 522px;">
		<div class="fs11 pb5">
			<div class="left">
			...
			<div class="right">
				<? if ( session::is_authenticated() && !blogs_comments_peer::instance()->has_rated($id, session::get_user_id())  && $comment['user_id']!=session::get_user_id() ) { ?>
					<span>
						<a href="javascript:;" onclick="blogsController.rateComment(this, <?=$comment['id']?>, true);"><?=tag_helper::image('common/up.gif', array('height' => 16, 'class' => 'vcenter'))?></a>
						<a href="javascript:;" onclick="blogsController.rateComment(this, <?=$comment['id']?>, false);"><?=tag_helper::image('common/down.gif', array('height' => 16, 'class' => 'vcenter'))?></a>
					</span>
				<? } ?>
		...
		<div class="comment_hdata<?=$comment['id']?> fs11 mb5 mt5 <?=$do_hide_comment ? 'hidden' : ''?>">
			...
			<? if ( session::has_credential('moderator') ||
				( ( $comment['user_id'] == session::get_user_id() ) && !$comment['childs'] && !db_key::i()->exists('negative_blog_comment:'.$id)) ||
				( session::has_credential('selfmoderator') && $post_data['user_id'] == session::get_user_id() ) ) { ?>
				<a href="javascript:;" onclick=" if ( confirm('Точно?') ) { $(this).parent().parent().parent().hide(); $.get('/blogs/delete_comment?id=<?=$comment['id']?>') } " class="dotted ml10"><?=t('Удалить')?></a>
			<? } ?>
		</div>
	</div>
	<div class="clear"></div>
