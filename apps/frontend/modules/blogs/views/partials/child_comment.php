...
<div class="fs11 mt10 p5" style="border: 1px solid #E4E4E4; background: #F9F9F9;">
	<? $do_hide_child_comment = $child_comment['rate'] <= -5; ?>

	<div class="comment_hdata<?=$child_comment['id']?> left <?=$do_hide_child_comment ? 'hidden' : ''?>">
	...
	<div style="margin-left: 60px;">
		<div class="left quiet">
			...
			<? if ( $do_hide_child_comment ) { ?>
				<a href="javascript:;" onclick="$('.comment_hdata<?=$child_comment['id']?>').show();" class="dotted ml10"><?=t('Показать')?></a>
			<? } ?>
		</div>
		<div class="right">
			<? if ( session::is_authenticated() && !blogs_comments_peer::instance()->has_rated($child_comment['id'], session::get_user_id()) && $child_comment['user_id']!=session::get_user_id() ) { ?>
				<span>
		...
		<br />
		<div class="mt10 comment_hdata<?=$child_comment['id']?> fs12 <?=$do_hide_child_comment ? 'hidden' : ''?>">
