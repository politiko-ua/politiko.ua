...
class freemail extends openinviter_base
	...
	public function login($user,$pass)
		...
		$res=$this->get("https://freemail.hu/levelezes/login.fm");
		if ($this->checkResponse("initial_get",$res))
			$this->updateDebugBuffer('initial_get',"https://freemail.hu/levelezes/login.fm",'GET');
		else
			{
			$this->updateDebugBuffer('initial_get',"https://freemail.hu/levelezes/login.fm",'GET',false);	
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
		...
		$form_action="https://belepes.t-online.hu/auth.html";
		$post_elements=array('.formId'=>'commands.PlusAuth',
							'backurl'=>'https://freemail.hu/levelezes/auth.fm?cmd=checkuser&page=levelezes',
							'cmd'=>'plusauth',
							'remoteform'=>1,
							'user'=>$user,
	...
	public function logout()
		...
		$res=$this->get('https://freemail.hu/levelezes/main.fm?page=logout',true);
		$this->debugRequest();
		$this->resetDebugger();
		$this->stopPlugin();
		return true;	
