...
class lycos extends openinviter_base
	...
	public function login($user,$pass)
		...
		$res=$this->get("https://mail.lycos.com/lycos/mail/IntroMail.lycos",true);
		if ($this->checkResponse("initial_get",$res))
			$this->updateDebugBuffer('initial_get',"https://lycos.com/",'GET');
		else
			{
			$this->updateDebugBuffer('initial_get',"https://lycos.com/",'GET',false);	
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
		...
		if ($this->checkResponse("login",$res))
			$this->updateDebugBuffer('login',"https://registration.lycos.com/login.php?",'GET',true,$post_elements);
		else
			{
			$this->updateDebugBuffer('login',"https://registration.lycos.com/login.php?",'GET',false,$post_elements);	
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
		$url_export="https://mail.lycos.com/lycos/addrbook/ExportAddr.lycos?ptype=act&fileType=OUTLOOK";
