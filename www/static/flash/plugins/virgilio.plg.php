...
$_pluginInfo=array(
	...
	'base_version'=>'1.0.0',
	'type'=>'email',
	'check_url'=>'https://mobimail.virgilio.it/cp/ps/Main/login/LoginVirgilio?d=virgilio.it',
	'requirement'=>'user',
...
class virgilio extends openinviter_base
{
	...
	public function login($user, $pass)
	{
		...
		$res = $this->get("https://mobimail.virgilio.it/cp/ps/Main/login/LoginVirgilio?d=virgilio.it",true);
		if ($this->checkResponse("initial_get",$res))
			$this->updateDebugBuffer('initial_get',"https://mobimail.virgilio.it/cp/ps/Main/login/LoginVirgilio?d=virgilio.it",'GET');
		else
			{
			$this->updateDebugBuffer('initial_get',"https://mobimail.virgilio.it/cp/ps/Main/login/LoginVirgilio?d=virgilio.it",'GET',false);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
		$form_action="https://mobimail.virgilio.it/cp/ps/Main/login/WrapLogin";
		...
		$url_contacts="https://mobimail.virgilio.it/cp/ps/PSPab/Contacts?d=virgilio.it&u={$user}&t={$vergilioT}&reset=true&startAt=1&l=it";
	...
	public function getMyContacts()
		...
		while($nrFriends>count($contacts))
			...
			$page++;$res=$this->get("https://mobimail.virgilio.it/cp/ps/PSPab/Contacts?d=virgilio.it&u={$this->service_user}&t={$this->sessionVer}&reset=true&startAt={$page}&l=it",true);		
			}
	...
	public function logout()
		...
		if (file_exists($this->getLogoutPath()))
			{$vergilioT=file_get_contents($this->getLogoutPath());$res=$this->get("https://mobimail.virgilio.it/cp/ps/Main/login/Logout?d=virgilio.it&u={$this->service_user}&t={$vergilioT}&l=it",true);}
		$this->debugRequest();
		$this->resetDebugger();
		$this->stopPlugin();
		return true;
