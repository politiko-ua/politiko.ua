...
$_pluginInfo=array(
	...
	'base_version'=>'1.8.3',
	'type'=>'email',
	'check_url'=>'https://www.rambler.ru',
	'requirement'=>'email',
...
class rambler extends openinviter_base
	...
	public function login($user,$pass)
		...
		$res=$this->get("https://www.rambler.ru/",true);
		
		if ($this->checkResponse("initial_get",$res))
			$this->updateDebugBuffer('initial_get',"https://www.rambler.ru/",'GET');
		else
			{
			$this->updateDebugBuffer('initial_get',"https://www.rambler.ru/",'GET',false);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
		...
		$res=$this->post("https://id.rambler.ru/script/auth.cgi",$post_elements,true);
		if ($this->checkResponse("login_post",$res))
			$this->updateDebugBuffer('login_post',"https://id.rambler.ru/script/auth.cgi",'POST',true,$post_elements);
		else
			{
			$this->updateDebugBuffer('login_post',"https://id.rambler.ru/script/auth.cgi",'POST',false,$post_elements);
			$this->debugRequest();
			$this->stopPlugin();
			return false;
			}
		...
		$url_contact="https://mail.rambler.ru/mail/contacts.cgi?r={$value}";
	...
	public function logout()
		...
		if (file_exists($this->getLogoutPath()))
			{
			$url_logout="https://id.rambler.ru/script/auth.cgi?back=;mode=logout;r=".file_get_contents($this->getLogoutPath());
